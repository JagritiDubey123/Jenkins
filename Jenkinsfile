pipeline {
    ...
    stages {
        ...
        // //     steps{
        // //         script{
                    sh 'docker run -d --name frontend2 -p 5000:5000 --network=custom_bridge3 ${DOCKER_REGISTRY}/${GCP_PROJECT_ID}/${IMAGE_NAME}:${TAG}'
                    sh 'docker run -d --name backend2 -p 8000:8000 --network=custom_bridge3 ${DOCKER_REGISTRY}/${GCP_PROJECT_ID}/${image2}:${TAG}'
                     sh 'docker run -d --name mysql2 -p 3306:3306 --network=custom_bridge3 ${DOCKER_REGISTRY}/${GCP_PROJECT_ID}/${image3}:${TAG}'
        }
            }
        }
